...
/**
 * Handle GET request to view a photo (served inline so it can be used directly
 * as an <img> source once fetched and converted to an object URL client-side).
 */
async function handleGet(req: NextRequest, authContext: AuthResult) {
  try {
    ...
    const decryptedBuffer = decryptFile(photoLog.storedName, PHOTOS_SUBDIR);
    ...
    return new NextResponse(decryptedBuffer.buffer.slice(decryptedBuffer.byteOffset, decryptedBuffer.byteOffset + decryptedBuffer.byteLength) as ArrayBuffer, {
      headers: {
        'Content-Type': photoLog.mimeType,
